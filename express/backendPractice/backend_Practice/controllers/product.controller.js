...
import { SubCategory } from "../models/productModel.js";
...
console.log("API Triggerred")
  try {
    ...
    const subCategory=new SubCategory({color,price,size,stock,sku,brand})

    // Create product
    const product = new Product({ name, description, category,subCategory:subCategory, images });
    await product.save();
    ...
    console.error(error);
    res.status(500).json({
      message: "Something went basit wrong.",
      status: 500,
      error: error.message
    });
  }
